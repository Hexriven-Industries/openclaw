...
run_ssh "PROD_TOKEN=\"\$(/usr/bin/plutil -extract EnvironmentVariables.OPENCLAW_GATEWAY_TOKEN raw -o - '${REMOTE_PROD_PLIST}')\"; OPENCLAW_CONFIG_PATH='${REMOTE_PROD_CONFIG}' OPENCLAW_STATE_DIR='${REMOTE_PROD_STATE}' OPENCLAW_GATEWAY_TOKEN=\"\$PROD_TOKEN\" '${REMOTE_PROD_BIN}' status --deep"

echo "  -> Remote prod channel probe"
run_ssh "PROD_TOKEN=\"\$(/usr/bin/plutil -extract EnvironmentVariables.OPENCLAW_GATEWAY_TOKEN raw -o - '${REMOTE_PROD_PLIST}')\"; OPENCLAW_CONFIG_PATH='${REMOTE_PROD_CONFIG}' OPENCLAW_STATE_DIR='${REMOTE_PROD_STATE}' OPENCLAW_GATEWAY_TOKEN=\"\$PROD_TOKEN\" '${REMOTE_PROD_BIN}' channels status --probe"
